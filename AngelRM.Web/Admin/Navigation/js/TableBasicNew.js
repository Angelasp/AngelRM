...
function deleteCurRow(index) {
    ...
    $.messager.confirm('确 认', '确认要删除选中的数据吗?', function (r) {
        if (r) {
            ...
            $.ajax({
                type: 'get',
                url: 'ashx/NavigationService.ashx?action=DelDB&id=' + raw.ID,
                cache: false,
                success: function (result) {
...
function Save(id) {
    var url = null;
    if (SaveStuate == "add") {
        url = 'ashx/NavigationService.ashx?action=SaveDB&method=add';
    }
    if (SaveStuate == "save") {
        url = 'ashx/NavigationService.ashx?action=SaveDB&method=modify&id=' + id + '';
    }
    if (id > 0) {
