...
function toolBarAddClick() {
    ...
    $('#ff').form('clear');
    $('#dd').dialog({ title: '新增系统参数', closed: false });
    $("#divShow").empty();
    $("#divShow").append(' <ul id="Data"></ul><a href="#">+ 添加</a>');
    $("#Data").easyinsert(); //最简单的应用 
...
function viewCurRecord(index) {
    ...
    $('#dd').dialog({ title: '编辑系统参数', closed: false });
    $('#id').val(raw.id);
    $('#ParaID').val(raw.ParaID);
    $('#ParaName').val(raw.ParaName);
    oldParaID = raw.ParaID;
    var v = raw.Data.split(',');
    var html = '';
    ...
    $("#Data").easyinsert({
        name: "Data",
        toplimit: 0,
        initValue: eval('[' + html.substring(0, html.length - 1) + ']')
    });
}
//
//行命令_"删除"
...
function deleteCurRow(index) {
    ...
    $.messager.confirm('确 认', '确认要删除选中的数据吗?', function (r) {
        if (r) {
            ...
            $.ajax({
                type: 'get',
                url: 'ashx/SystemInfo.ashx?action=DelDB&id=' + raw.id,
                cache: false,
                success: function (result) {
...
function Save(id) {
    var url = null;
    if (SaveStuate == "add") {
        url = 'ashx/SystemInfo.ashx?action=AddSysparameter';
    }
    if (SaveStuate == "save") {
        url = 'ashx/SystemInfo.ashx?action=UpSysparameter&id=' + id + '';
    }
    if (id > 0) {
